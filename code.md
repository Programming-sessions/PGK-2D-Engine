...
Ôªø#include "src/engine/Engine.h"
...
int main() {
    ...
    engine->setClearColor(al_map_rgb(0, 0, 0));  // Czarne t≈Ço
    ...
    player->setPosition(100, 100);  // PoczƒÖtkowa pozycja na ≈õrodku ekranu
    ...
    // Inicjalizacja renderera prymityw√≥w
    ...
    for (int i = 0; i < 3; i++) {
        ...
        enemy->setPosition(
            200.0f + i * 500.0f,  // R√≥≈ºne pozycje X
            200.0f + i * 300.0f   // R√≥≈ºne pozycje Y
        );
...
// Struktura przechowujπca dane pojedynczej klatki animacji
struct AnimationFrame {
    int x;          // Pozycja X w sprite sheet
    int y;          // Pozycja Y w sprite sheet
    int width;      // SzerokoúÊ klatki
    int height;     // WysokoúÊ klatki
...
Engine::Engine() :
    ...
    // Inicjalizacja stanÛw klawiszy i myszy
...
bool Engine::init() {
    ...
    // Inicjalizacja poszczegÛlnych komponentÛw
    if (!initAllegro()) {
...
bool Engine::initDisplay() {
    // Ustawienie parametrÛw wyúwietlania
...
void Engine::startGameLoop(std::function<void(float)> updateCallback,
    ...
    while (running) {
        ...
        if (redraw && al_is_event_queue_empty(eventQueue)) {
            ...
            // Wywo≥anie funkcji aktualizujπcej logikÍ gry
            if (updateCallback) {
            ...
            // Wywo≥anie funkcji renderujπcej
            if (renderCallback) {
                renderCallback();
            }
...
void Engine::shutdown() {
    ...
    // Zniszczenie zasobÛw Allegro w odwrotnej kolejnoúci od utworzenia
    if (timer) {
    ...
    // Deinicjalizacja dodatkÛw Allegro
...
public:
    ...
    // G≥Ûwna pÍtla i obs≥uga zdarzeÒ
...
void Logger::log(LogLevel level, const std::string& message) {
    ...
    // Bezpieczna konwersja czasu do ciπgu znakÛw
    char timeStr[26];
    ctime_s(timeStr, sizeof(timeStr), &time);
    std::string timeString(timeStr);
    // UsuniÍcie znaku nowej linii, ktÛry dodaje ctime_s
...
void PrimitiveRenderer::scanlineFill(int y, int x1, int x2) {
    // Upewniamy siÍ, øe x1 < x2
    if (x1 > x2) std::swap(x1, x2);

    // Rysujemy liniÍ poziomπ punkt po punkcie
    for (int x = x1; x <= x2; x++) {
...
void PrimitiveRenderer::fillTriangle(int x1, int y1, int x2, int y2, int x3, int y3) {
    ...
    if (y2 == y3) { // TrÛjkπt z p≥askπ podstawπ na dole
    ...
    else if (y1 == y2) { // TrÛjkπt z p≥askπ podstawπ na gÛrze
...
void PrimitiveRenderer::drawRectangle(const Point2D& topLeft, float width, float height, bool filled) {
    ...
    // Normalizacja wymiarÛw - obs≥uga ujemnych wartoúci
    if (w < 0) {
        x += w;
        w = -w;
    }
    ...
    if (filled) {
        // Wype≥niamy prostokπt linia po linii
        for (int currY = y; currY <= y + h; currY++) {
            scanlineFill(currY, x, x + w);
        }
    }

    // Rysujemy obrys prostokπta
...
void PrimitiveRenderer::drawCircle(const Point2D& center, float radius, bool filled) {
    ...
    // Algorytm Bresenhama dla okrÍgu
    int x = 0;
    int y = intRadius;
    int d = 3 - 2 * intRadius;
...
private:
    ...
    // Metody pomocnicze do rysowania okrÍgÛw
...
void Rectangle::rotate(float angle, const Point2D& center) {
    // Dla prostokπta rotacja jest bardziej skomplikowana
    // Trzeba przekszta≥ciÊ go w cztery punkty, obrÛciÊ i utworzyÊ nowy prostokπt
    ...
    // Po obrocie moøemy potrzebowaÊ przekalkulowaÊ wymiary
    // To jest uproszczona wersja - w praktyce moøe wymagaÊ dodatkowych obliczeÒ
...
void Circle::scale(float sx, float sy) {
    // Dla ko≥a uøywamy tylko jednego wspÛ≥czynnika skalowania
    float scale = (sx + sy) / 2.0f;  // Moøna teø uøyÊ std::max(sx, sy)
    radius *= scale;
}
...
public:
    ...
    // Transformations
    void translate(float dx, float dy);
    void scale(float sx, float sy);  // Dla koÔøΩa sx i sy powinny byÔøΩ rÔøΩwne
    // KoÔøΩo nie potrzebuje rotacji, bo jest symetryczne
...
void Sprite::addAnimation(Animation* animation) {
    if (animation) {
        // Jeúli animacja o tej nazwie juø istnieje, usuÒ jπ
        ...
        // Jeúli to pierwsza animacja, ustaw jπ jako aktualnπ
        if (!currentAnimation) {
...
void Sprite::updateAnimation(float deltaTime) {
    if (currentAnimation) {
        ...
        // Aktualizacja ürÛd≥owego prostokπta na podstawie aktualnej klatki
...
void Sprite::draw() {
    ...
    // Rysowanie z uwzglÍdnieniem przezroczystoúci i aktualnej klatki animacji
    ...
    // PrzywrÛcenie poprzedniej transformacji
...
TextureManager::TextureManager() {
    // Inicjalizacja dodatku do obs≥ugi obrazÛw
    if (!al_init_image_addon()) {
...
ALLEGRO_BITMAP* TextureManager::loadTexture(const std::string& path) {
    // Sprawdü czy tekstura juø jest za≥adowana
    if (isTextureLoaded(path)) {
        return textures[path];
    }

    // Za≥aduj nowπ teksturÍ
...
private:
    ...
    // Mapa przechowujπca tekstury (úcieøka -> bitmap)
    std::map<std::string, ALLEGRO_BITMAP*> textures;

    // Logger do obs≥ugi b≥ÍdÛw
    Logger logger;
...
Enemy::Enemy()
    ...
    , maxSpeed(200.0f)        // TrochÍ wolniejszy niø gracz
    , acceleration(300.0f)
    , deceleration(600.0f)
    , health(100.0f)
    , maxHealth(100.0f)
...
void Enemy::update(float deltaTime) {
    ...
    // Zapisz poprzedniπ pozycjÍ przed ruchem
...
void Enemy::updateAI(float deltaTime) {
    ...
    if (isPlayerInRange()) {
        ...
        // Zatrzymaj siÍ jeúli gracz jest poza zasiÍgiem
...
private:
    ...
    float detectionRange;     // ZasiÍg wykrywania gracza
    float maxSpeed;           // Maksymalna prÍdkoúÊ
...
void Entity::update(float deltaTime) {
    ...
    // Aktualizacja pozycji na podstawie prÍdkoúci
    position.setX(position.getX() + velocity.getX() * deltaTime);
    position.setY(position.getY() + velocity.getY() * deltaTime);
...
Map::Map(int width, int height, int tileSize)
    ...
    , height(height)
    , tileSize(tileSize)
    , wallColor(al_map_rgb(100, 100, 150))  // Kolor úcian/przeszkÛd
{
...
bool Map::init(PrimitiveRenderer* renderer) {
    // Tworzymy teksturÍ o rozmiarze mapy
    ...
    // Ustawiamy teksturÍ mapy jako cel renderowania
...
void Map::initializeMapObjects() {
    ...
    // Dodanie kilku mniejszych przeszkÛd
    for (int i = 0; i < 5; i++) {
        Rectangle obstacle(
            Point2D(
...
void Map::drawMapObjects(PrimitiveRenderer* renderer) {
    ...
    // Rysowanie wszystkich obiektÛw z kolizjπ
    for (const auto& collider : colliders) {
        // Rysowanie wype≥nionego prostokπta
...
void Map::generateMapTexture(PrimitiveRenderer* renderer) {
    // Najpierw rysujemy siatkÍ
    drawGrid(renderer);

    // NastÍpnie rysujemy obiekty mapy
...
Player::Player()
    : Entity()
    , maxSpeed(300.0f)  // Piksele na sekundÍ
    , acceleration(400.0f)
    , deceleration(800.0f)
    , isMoving(false)
    , health(100.0f)
...
bool Player::loadResources() {
    ...
    // Dodawanie klatek animacji (4 rzÍdy po 5 klatek)
    for (int row = 0; row < 4; row++) {
        for (int col = 0; col < 5; col++) {
            idleAnimation->addFrame(
                col * FRAME_WIDTH,
...
void Player::handleInput(float deltaTime) {
    ...
    // Obliczanie wektora kierunku na podstawie wciúniÍtych klawiszy
    float dirX = 0.0f;
    float dirY = 0.0f;
